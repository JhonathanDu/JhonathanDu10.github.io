        ...
        <div class="col-md-6">
            <form class="form-group" id="calcularDyAForm">
                <div class="card-rounded">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            <label for="alturaAntena" class="form-label custom-label">Altura de la antena (metros):</label>
                        </div>
                        <div class="col">
                            <input type="number" step="any" class="form-control form-control-sm custom-input" id="alturaAntena" name="alturaAntena" required>
                        </div>
                    </div>

                    <input type="submit" value="Calcular d y a" class="custom-button" onclick="mostrarResultado()">
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('calcularDyAForm').addEventListener('submit', function(event) {
        ...
        var alturaAntena = parseFloat(document.getElementById('alturaAntena').value);
        
        if (isNaN(r) || isNaN(alturaAntena) || r < 0 || alturaAntena < 0) {
            alert("Por favor ingrese valores válidos para 'r' y 'a'.");
            return;
        }
        ...
        if (a >= r && d >= r) {
            ...
            document.getElementById('resultadoDA').innerHTML = 'La distancia horizontal (d) es: ' + d.toFixed(2) + ' metros, y la distancia vertical (a) es: ' + a.toFixed(2) + ' metros.';
        }
    });
